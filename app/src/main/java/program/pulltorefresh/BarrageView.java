...
public class BarrageView extends RelativeLayout {
    ...
    private Random random = new Random(System.currentTimeMillis());//得到随机数
    ...
    private void init() {
        ...
        int duration = (int) ((BARRAGE_GAP_MAX_DURATION - BARRAGE_GAP_MIN_DURATION) * Math.random());//弹幕产生的时间
    }

    //在这个方法中防止位置重叠，因为在这个方法中通过View的getMeasuredHeight()得到的高度不为空
    @Override
    ...
    public int getLineHeight() {
        ...
        item.textView = new TextView(context);
    ...
    private Animation generateTranslateAnim(BarrageItem item, int leftMargin) {
...
//得到text文本的大小
    private int getTextWidth(BarrageItem item, String tx, int sz) {
        Rect bounds = new Rect();
        TextPaint paint;
        paint = item.textView.getPaint();
        paint.getTextBounds(tx, 0, tx.length(), bounds);
